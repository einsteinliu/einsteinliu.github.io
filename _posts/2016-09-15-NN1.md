...
$$\sigma(\sum_ix_i\cdot \omega_i+b)=P(y_i =1\vert x_i;\omega)$$ . 
...
$$f(x)=max(0,x)$$, combining the neuron: $$f(x) = Max(0,\omega\cdot x+b)$$
...
$$Maxout(x)=\displaystyle\max_{i=1\dots k}(f_i(x))=\max_{i=1\dots k}(\omega_i\cdot x + b_i)$$              : 
