...
对于一组$${x_i,y_i}$$，通过ω得出一个prediction：$$f_ω (x_i)$$

使用普通的hinge loss function作为损失函数，就会得出以下优化问题：

$$
\displaystyle argmin_ω{\left\{  \frac{1}{2}|ω|^2 +C\sum_{i=1}^Nmax(0,1-y_i\cdot f_\omega(x_i))  \right\} }
$$

作为一个普通的svm问题，这个问题很好解决

但在此例中，我们引入了latent value h，$$f_ω (x_i)$$不再是一个简单的$$ω\cdot x_i+b$$这样的线性函数

而是：

$$f_ω (x_i )=\max_{h\inΗ}⁡ω\cdotΦ(x_i,h)$$

也就是说，prediction变成了一个和latent value有关的非线性离散函数,
上述问题变得难以解决。

但可以发现h和ω其实没有关系，所以可以将他们看做两个无关变量，通过坐标下降法求解


**第一步，固定ω，更新**$$h_i:h_1…h_N$$

也就是，在已知ω的情况下，通过$$f_ω (x_i )=\max_{h\inΗ}⁡ω\cdotΦ(x_i,h)$$算出一个$$h_i$$

然后确定了这个$$h_i$$，保持其不变，$$f_ω (x_i)$$变成了一个ω的普通线性凸函数

**第二步,固定$$h_i:h_1…h_N$$，更新ω：**

所以对于每一个i,在基于已有的ω计算出了$$h_i$$之后，$$\max⁡(0,1−y_if_ω (x_i))$$成了凸函数

**而凸函数之和也是凸函数**，所以把i=1…N的所有$$max⁡(0,1−y_if_ω (x_i))$$加起来，仍然是凸函数
